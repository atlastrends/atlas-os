...
def _do_update() -> None:
    ...
    # 2) Prepara o commit (pagina + lista do robo + backup do historico).
    _run(["git", "add", "docs/index.html", "docs/produtos.json", "docs/_bio_historico.json"])

    # 3) Se nada mudou, nao commita (evita commit vazio).
    diff = _run(["git", "diff", "--cached", "--quiet", "--", "docs/index.html", "docs/produtos.json", "docs/_bio_historico.json"])
